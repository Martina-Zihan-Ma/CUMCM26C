...
附件3每个 `issue_time` 给出未来第1至24小时整点值；清洗表仅在 +60 至 +1440 分钟内线性插值。发布后首小时的五个十分钟点，以发布时间已观测到的光伏功率和本版本 +1 小时预测线性插值，不使用未来实际值。未来负荷没有题给预报。根据历史数据中明显的周周期性，采用最多过去4个同星期几、同一10分钟时刻的实际负荷进行递减加权（0.4/0.3/0.2/0.1，并对可用样本重新归一化）；年初尚无同星期几历史时，使用附件1标准日负荷曲线作为冷启动。所有历史源均严格早于当前 `issue_time`。
...
完整365天滚动执行表为 `question3_full_period_rolling_schedule.csv`，题目给定格式的汇总工作簿为 `result3.xlsx`。四个指定日的调度、SOC、预测误差和更新前后计划图位于 `outputs/question3/figures/`。
...
局限包括：题面未完全展开下调结算的会计口径，本文采用“取消全价购买并支付50%违约费”的净结算解释；负荷预测采用同星期几历史加权的轻量基线，未额外训练复杂预测模型；SOC离散存在小幅数值误差；预测过剩时可能出现已购电溢出，本文单列处理。若官方另有下调结算口径，只需替换 `settlement_cost`。
