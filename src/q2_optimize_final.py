#!/usr/bin/env python3
"""
Q2 Step 2 — final day-ahead stochastic optimization.

Risk design:
1. SERVICE_LEVEL quantile constraint for intraday net-demand uncertainty.
2. Dynamic terminal reserve calibrated at the fixed 85% empirical risk quantile.
...
# Risk control: service quantile + dynamic reserve.
SERVICE_LEVEL = 0.95
RESERVE_Q = 0.80
...
def fit_reserve_rule(calibration: pd.DataFrame) -> ReserveRule:
    risk = calibration["risk_kwh"].to_numpy(float)
    return ReserveRule(
        q50=float(np.quantile(risk, RESERVE_MEDIAN_Q)),
        q85=float(np.quantile(risk, RESERVE_Q)),
...
def reserve_for_day(
    ...
    center = SOC_MIN + (rule.q85 / ETA_D) * ratio
    reserve = center - (rule.q85 - rule.q50) / ETA_D
...
def warm_up_january(
    ...
    for date in pd.date_range(JAN_START + pd.Timedelta(days=1), JAN_END):
        load_hat = january_load_forecast(actual, date)
        pv_hat = january_pv_forecast(actual, date)
        rule = causal_january_rule(calibration, date)
        reserve = (
...
def main() -> None:
    ...
    rule = fit_reserve_rule(calibration)
    feb1_soc, january_residuals, january_warmup = warm_up_january(
        price, actual, calibration
    )
