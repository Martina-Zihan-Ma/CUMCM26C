...
def _state_grid(parameters: StorageParameters, soc0: float, terminal_soc: float) -> np.ndarray:
    p = parameters
    regular = np.arange(p.soc_min_kwh, p.soc_max_kwh + p.soc_step_kwh / 2, p.soc_step_kwh)
    states = np.unique(np.r_[regular, p.soc_min_kwh, p.soc_max_kwh, soc0, terminal_soc])
...
def optimize_dispatch(
    ...
    if not (p.soc_min_kwh <= terminal_soc_kwh <= p.soc_max_kwh):
    ...
    terminal_idx = int(np.argmin(np.abs(states - terminal_soc_kwh)))
    if abs(states[start_idx] - soc0_kwh) > 1e-8 or abs(states[terminal_idx] - terminal_soc_kwh) > 1e-8:
        raise RuntimeError("SOC 初末状态没有进入状态网格")
    ...
    if not np.isfinite(dp[terminal_idx]):
        raise RuntimeError("给定 SOC 网格和末端状态下窗口不可行")
    ...
    plan_cost, up, down, adjustment = settlement_cost(g, price, base)
    return pd.DataFrame(
        {
